...
export default function TftTournamentDetailPage() {
  ...
  if (loading) {
    return (
      <main className="min-h-screen bg-[#0e1525]">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 text-[#8a9bb0] text-center">{t('tft.loading')}</div>
        <Footer />
      </main>
    );
  }
  if (!tournament) {
    return (
      <main className="min-h-screen bg-[#0e1525]">
        <Nav active="tournaments" />
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 text-[#8a9bb0] text-center">{t('tft.tournaments.notFound')}</div>
        <Footer />
      </main>
    );
  }

  const tierColor = tournament.tier ? (TIER_COLORS[tournament.tier] || '#8a9bb0') : '#8a9bb0';
  const locale = LOCALE_MAP[lang];
  const dateFmt = (s: string | null) => s ? new Date(s).toLocaleDateString(locale, { day: '2-digit', month: 'long', year: 'numeric' }) : '—';
  const statusColor = tournament.status === 'live' ? '#e44040' : tournament.status === 'upcoming' ? '#3ecf8e' : '#8a9bb0';
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-[#0d1526] border border-[#1e2a3a] rounded-lg p-5 mb-5 mt-2">
          <div className="flex items-start gap-4 flex-wrap">
            ...
            <div className="flex-1 min-w-0">
              ...
              {tournament.format && (
                <div className="text-[#4a5a70] text-xs mt-1">{tournament.format}</div>
              )}
            </div>
        ...
        {tournament.status === 'live' && tournament.twitch_channel && (
          <div className="bg-[#0d1526] border border-[#1e2a3a] rounded p-3 mb-5">
            <div className="text-[#8a9bb0] text-xs uppercase tracking-widest mb-2">{t('tft.tournaments.liveStream')}</div>
        ...
        {tournament.results && tournament.results.length > 0 && (
          <section className="bg-[#0d1526] border border-[#1e2a3a] rounded overflow-hidden">
            ...
            <div className="hidden sm:grid grid-cols-[3rem_1fr_8rem_5rem_6rem] gap-2 px-4 py-2 text-[10px] uppercase text-[#4a5a70] border-t border-[#1e2a3a]">
            ...
            {tournament.results.map(r => {
              const placeColor = r.placement === 1 ? '#f0c040' : r.placement === 2 ? '#cfd6dc' : r.placement === 3 ? '#cd7f32' : '#8a9bb0';
              ...
              return (
                ...
                >
                  ...
                  <div className="hidden sm:block text-[#8a9bb0] truncate">{r.team || '—'}</div>
                  <div className="hidden sm:block text-[#8a9bb0]">{r.country || '—'}</div>
                  <div className="flex sm:block items-center justify-between mt-1 sm:mt-0 sm:text-right tabular-nums">
                    <span className="text-[#4a5a70] text-[10px] sm:hidden">{r.team}{r.team && r.country ? ' · ' : ''}{r.country}</span>
