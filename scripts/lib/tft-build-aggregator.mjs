...
// Cluster a participant's board into a deterministic key. Delegiert an die
// Single-Source-Lib (scripts/lib/tft-classify-comp.mjs) mit Augment-Suffix-
// Mode an (Aggregator-Format `<trait>@<level>_<carry>~<aug>`).
function classifyComp(participant, opts = {}) {
  return classifyCompUnified(participant, { currentSet: opts.currentSet, withAugmentSuffix: true });
}
