...
import { loadCompGuidesBundle } from '../../lib/tft-comp-guides';
import {
  buildAugmentSlotMap,
  augmentStageInfo,
  augmentStageSortKey,
...
export default function TftAugmentsReferencePage() {
  ...
  // Slot-Map einmalig pro Bundle-Load — pro Augment Slot-Counts aus den 33
  // kuratierten Comp-Guides. Lookup ist O(1) im sort/filter-Hot-Path.
  ...
  const filtered = useMemo(() => {
    ...
    return augments
      ...
      .filter(a => {
        ...
        return augmentStageInfo(a.apiName, slotMap, a.tier).stage === stageFilter;
      })
      .filter(a => {
        if (!q) return true;
      ...
      .sort((a, b) => {
        if (sortMode === 'stage') {
          // Stage-Sort: 2-1 → 3-2 → 4-2. Tier-Heuristik-Augments sortieren
          // innerhalb derselben Stage hinter Guides-basierte (siehe lib).
          const ka = augmentStageSortKey(augmentStageInfo(a.apiName, slotMap, a.tier));
          const kb = augmentStageSortKey(augmentStageInfo(b.apiName, slotMap, b.tier));
          if (ka !== kb) return ka - kb;
          // Tiebreaker: Tier asc, dann Name asc
  ...
  }, [augments, tierFilter, stageFilter, query, lang, slotMap, sortMode]);
  ...
  return (
    <main className="min-h-screen bg-[#0e1525]">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-2 pb-6">
        ...
        {/* Stage-Filter + Sort-Toggle. Stage-Daten kommen aus den 33
            kuratierten tftacademy-Comp-Guides (Slot-Position) + Fallback
            auf Tier-Heuristik (Silver→2-1, Gold→3-2, Prismatic→4-2). */}
        <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {assets && filtered.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filtered.map(a => {
              ...
              const stageData = augmentStageInfo(a.apiName, slotMap, a.tier);
                    ...
                    <div className="flex items-center gap-1.5 mt-1">
                      <span
                        ...
                        style={{
                          ...
                          opacity: stageData.source === 'tier-heuristic' ? 0.65 : 1,
                        }}
                        title={
                          stageData.distribution
                            ? `Stage ${stageData.stage} · ${stageData.distribution[0]}×2-1, ${stageData.distribution[1]}×3-2, ${stageData.distribution[2]}×4-2 (aus Guides)`
