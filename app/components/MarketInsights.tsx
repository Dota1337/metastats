...
const TREND_ICONS: Record<string, { icon: string; color: string }> = {
  ...
  stable: { icon: '\u2192', color: 'text-fg-secondary' },
    ...
    <div className="bg-surface-base border border-border-subtle rounded-lg overflow-hidden mb-4">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-fg-muted text-[10px]">{t('mi.subtitle')}</div>
          </div>
        </div>
      ...
      {expanded && (
        <div className="px-4 pb-4">
          {/* Tabs */}
          <div className="flex gap-1 bg-surface-raised rounded-lg p-0.5 mb-3">
            <button
              onClick={() => setTab('transfers')}
              className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                tab === 'transfers' ? 'bg-surface-overlay text-white shadow-sm' : 'text-fg-secondary hover:text-white'
              }`}
            >
              {t('mi.transferRadar')}
            </button>
            <button
              ...
              className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                tab === 'anomalies' ? 'bg-surface-overlay text-white shadow-sm' : 'text-fg-secondary hover:text-white'
              }`}
            >
              {t('mi.anomalies')}
            </button>
          ...
          {loading ? (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-fg-secondary text-xs bg-surface-overlay px-1.5 py-0.5 rounded">{tp.role}</span>
                          {playerLink && (
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-fg-muted text-[10px]">{t('mi.transferProb')}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-3">
                          <div>
                            <div className="text-fg-secondary text-xs">{tp.currentTeam}</div>
                            {tp.teamRegion && <div className="text-fg-muted text-[10px]">{tp.teamRegion}</div>}
                          </div>
                        </div>
                        <div className="flex items-center gap-3">
                          {tp.winrate && (
                            <div className="text-center">
                              ...
                              <div className="text-fg-muted text-[10px]">{t('mv.winrate')}</div>
                            </div>
                          )}
                          {tp.tier && (
                            <div className="text-center">
                          ...
                          {tp.teamAvgPlace && (
                            <div className="text-center">
                              ...
                              <div className="text-fg-muted text-[10px]">{t('mi.teamPlace')}</div>
                            </div>
                          )}
                          <div className="text-center">
                            <div className={`text-xs font-medium ${
                              !tp.contractEnd ? 'text-fg-muted' :
                            ...
                            <div className="text-fg-muted text-[10px]">{t('mi.contractUntil')}</div>
                          </div>
                        </div>
                      </div>

                      {/* Probability bar */}
                      <div className="w-full h-1.5 bg-surface-overlay rounded-full mb-2">
                        <div
                      ...
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {directionLabel && (
                          <span className={`${directionColor} text-[10px] font-medium bg-surface-base px-2 py-0.5 rounded-full border border-border-subtle flex-shrink-0`}>
                            {directionLabel}
                          </span>
                        )}
                      </div>
            ...
            <div className="space-y-2">
              {anomalies.length === 0 ? (
                <div className="text-fg-muted text-xs text-center py-6">{t('mi.noAnomalies')}</div>
              ) : (
                      ...
                      <div className="flex items-center gap-4 flex-wrap">
                        ...
                        {a.summonerLevel != null && (
                          <div className="text-center">
                            ...
                            <div className="text-fg-muted text-[10px]">{t('player.level')}</div>
                          </div>
                        )}
                      </div>
                    </Wrapper>
