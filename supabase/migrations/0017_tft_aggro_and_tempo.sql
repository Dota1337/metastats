-- TFT Sprint 2 schema (2026-05-22)
-- Aggro-Index per comp + Leveling-Tempo per comp/final-level + per-bucket
-- breakdown for Skill-Cap-Index.

alter table tft_daily_comp_stats
  add column if not exists sum_players_eliminated bigint not null default 0,
  add column if not exists level_dist jsonb not null default '{}'::jsonb,
  add column if not exists level_sum_last_round jsonb not null default '{}'::jsonb;
