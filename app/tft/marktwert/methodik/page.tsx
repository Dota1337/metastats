  ...
  return (
    <main className="min-h-screen bg-surface-page">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-2 pb-10">
        <p className="text-fg-secondary text-sm mb-6">{t('tft.mv.method.intro')}</p>
        ...
        {qa.map(item => (
          <section key={item.id} id={item.id} className="scroll-mt-24 mb-6">
            ...
            <p className="text-fg-secondary text-sm leading-relaxed">{item.a}</p>
          </section>
        ))}
        ...
        <section id="signals" className="scroll-mt-24 mb-6">
          ...
          <p className="text-fg-secondary text-sm leading-relaxed mb-3">{t('tft.mv.method.a.signals')}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {SIGNALS.map(s => (
              <div
                key={s.key}
                id={s.key}
                className="scroll-mt-24 bg-surface-base border border-border-subtle rounded p-3"
              >
                <div className="flex items-baseline justify-between gap-2 mb-1">
                  <span className="text-white font-medium text-sm">{t(s.labelKey as TranslationKey)}</span>
                  <span className="text-fg-muted text-xs tabular-nums flex-shrink-0">{s.weight}%</span>
                </div>
                <p className="text-fg-secondary text-xs leading-relaxed">{t(`tft.mv.method.sig.${s.key}` as TranslationKey)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section id="multiplier" className="scroll-mt-24 mb-6">
          ...
          <p className="text-fg-secondary text-sm leading-relaxed">{t('tft.mv.method.a.mult')}</p>
        </section>
        ...
        <section className="bg-surface-base border border-border-subtle rounded p-4">
          ...
          <p className="text-fg-secondary text-sm leading-relaxed">{t('tft.mv.method.a.example')}</p>
        </section>
      </div>
      <Footer />
    </main>
