-- 0033: Carry-Star-Outcome auf tft_daily_comp_stats (Sprint W3-A)
--
-- Pro-Frage: "Wenn der Carry-Unit 3★ erreicht, was ist die avg-Platzierung?"
-- Reroll-Comps performen drastisch besser bei 3★ als bei 2★; Fast-8-Comps
-- erreichen 3★ selten, gewinnen aber zuverlässig wenn doch. carry_star_dist
-- ist jsonb in der Form { "1": {games, sumPlacement, top4, top1}, "2": …, "3": … }.
--
-- Migration ist ADDITIV — der laufende Aggregator schreibt das Feld ab dem
-- nächsten Crawl, ältere Rows haben dann '{}'. Die Detail-RPC liefert den
-- gemergten Tagesverlauf, die Liste bleibt UNVERÄNDERT (List-Path nutzt
-- get_tft_comp_stats_list, der kein carry_star_dist braucht).

alter table tft_daily_comp_stats
  add column if not exists carry_star_dist jsonb not null default '{}'::jsonb;

-- Postgres can't ALTER a function's return signature in-place — drop & rebuild.
